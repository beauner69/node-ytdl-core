...
exports.getBasicInfo = async (id, options) => {
  ...
  if (
    !player_response ||
    (!player_response.streamingData && !isRental(player_response))
  ) {
    // If the video page doesn't work, maybe because it has mature content.
    // and requires an account logged in to view, try the embed page.
    let [embedded_player_response, embedbody] = await retryFn(
      getEmbedPage,
      [id, options],
      retryOptions
    );
...
const isRental = (player_response) => {
  ...
  return (
    ...
    /requires payment/.test(playability.reason)
  );
};

const getWatchURL = (id, options) =>
  `${VIDEO_URL + id}&hl=${options.lang || "en"}&bpctr=${Math.ceil(
    Date.now() / 1000
  )}`;
...
const retryFn = async (fn, args, options) => {
  ...
  while (currentTry <= options.maxRetries) {
    try {
      ...
      if (
        err instanceof miniget.MinigetError ||
...
const getEmbedURL = (id, options) =>
  `${EMBED_URL + id}?hl=${options.lang || "en"}`;
const getEmbedPage = async (id, options) => {
  ...
  let jsonStr = utils.between(
    body,
    /(['"])PLAYER_(CONFIG|VARS)\1:\s?/,
    "</script>"
  );
  let config;
  if (!jsonStr) {
  ...
  let player_response =
    (config.args &&
      (config.args.player_response || config.args.embedded_player_response)) ||
...
const getVideoInfoPage = async (id, options, info) => {
  ...
  return parseJSON(
    "get_video_info `player_response`",
    moreinfo.player_response || info.playerResponse
  );
};
...
exports.getInfo = async (id, options) => {
  ...
  if (info.formats.length) {
    info.html5player =
      info.html5player || getHTML5player(await getWatchPage(id, options));
    if (!info.html5player) {
