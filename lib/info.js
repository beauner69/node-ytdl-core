...
// const sax = require("sax"); // ZEROLABS
const miniget = require("./_miniget"); // ZeroLabs
const util = require("./util");
// const extras = require("./info-extras"); // ZEROLABS
...
exports.getBasicInfo = async (id, options) => {
  ...
  const watchPageURL = `${VIDEO_URL + id}&${params}&bpctr=${Math.ceil(
    Date.now() / 1000
  )}`;
  ...
  const setIdentityToken = async (key) => {
    let token = await exports.cookieCache.getOrSet(key, async () => {
      let watchPageBody = await (
        await miniget.promise(watchPageURL, reqOptions)
      ).text();
  ...
  let body = await (await miniget.promise(jsonEndpointURL, reqOptions)).text();
  let info;
  try {
    let parsedBody = JSON.parse(body);
    if (
      parsedBody.reload === "now" &&
      !reqOptions.headers["x-youtube-identity-token"]
    ) {
      await setIdentityToken("browser");
      body = await (await miniget.promise(jsonEndpointURL, reqOptions)).text();
      parsedBody = JSON.parse(body);
    }
    if (!Array.isArray(parsedBody)) {
      throw Error(
        "Cookie header used in request, but unable to retrieve video metadata"
      );
    }
  ...
  if (!info.player) {
    ...
    body = await (
      await miniget.promise(embedUrl, options.requestOptions)
    ).text();
    ...
    if (
      ...
      !config.args.embedded_player_response &&
      playErr
    ) {
      throw playErr;
    }
...
const gotConfig = async (id, options, info, body) => {
  ...
  if (!player_response) {
    ...
    let morebody = await (
      await miniget.promise(url, options.requestOptions)
    ).text();
...
exports.getInfo = async (id, options) => {
  ...
  // ZEROLABS NO POINT DOING THE DASH THING
  /*
...
const getM3U8 = async (url, options) => {
  ...
  let body = await miniget.promise(url, options.requestOptions).text();
  let formats = {};
  body
    .split("\n")
...
// ZEROLABS ADDITIONAL:
exports.getInfoFromYTID = (sYTID) => {
  return exports.getInfo(sYTID, {});
  // return new Promise((resolve, reject) => {
  //   exports.getFullInfo(sYTID, {}, (err, info) => {
