...
const getHTMLWatchURL = (id, options) =>
...
const getWatchJSONURL = (id, options) =>
  `${getHTMLWatchURL(id, options)}&pbj=1`;
const getJSONWatchPage = async (id, options) => {
  ...
  info.player_response = findPlayerResponse(
    'watch.json `player_response`',
    info
  );
...
exports.getInfo = async (id, options) => {
  ...
  if (info.formats.length) {
    info.html5player =
      info.html5player ||
      getHTML5player(await getHTMLWatchPageBody(id, options));
    if (!info.html5player) {
