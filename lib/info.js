...
exports.getBasicInfo = async (id, options) => {
  ...
  const setIdentityToken = async (key, throwIfNotFound) => {
    ...
    let token = await exports.cookieCache.getOrSet(key, async () => {
      ...
      if (!match && throwIfNotFound) {
        throw Error(
          "Cookie header used in request, but nunable to find YouTube identity token"
        );
      }
  ...
  if (!info.player) {
    ...
    let html5playerRes = /<script\s+src="([^"]+)"\s+name="player_ias\/base"\s*>/.exec(
      body
    );
...
*/ // ZEROLABS
  ...
  // ZEROLABS info.related_videos = extras.getRelatedVideos(info);
  info.html5player =
    info.html5player ||
