...
exports.getBasicInfo = async (id, options) => {
  ...
  let url = `${VIDEO_URL + id}&${params}&bpctr=${Math.ceil(
    Date.now() / 1000
  )}&pbj=1`;
  ...
  let info;
  try {
    info = JSON.parse(body).reduce(
      (part, curr) => Object.assign(curr, part),
      {}
    );
  } catch (err) {
...
const gotConfig = async (id, options, info, body) => {
  ...
  if (moreinfo.status === "fail") {
    throw Error(
      `Code ${moreinfo.errorcode}: ${util.stripHTML(moreinfo.reason)}`
    );
...
exports.getInfoFromYTID = (sYTID) => {
  return exports.getFullInfo(sYTID, {});
